...
class Git:
    ...
    def __init__(gh, bot):
        gh.bot = bot
        gh.session = aiohttp.ClientSession()
    ...
    async def githubusername(gh):
        '''Returns Github Username'''
        async with gh.session.get('https://api.github.com/user', headers={"Authorization": f"Bearer {gh.githubtoken}"}) as resp: #get username 
            if 300 > resp.status >= 200:
                return (await resp.json())['login']
            if resp.status == 401: #invalid token!
                return None

    async def starred(gh, repo):
        async with gh.session.get('https://api.github.com/user/starred/' + repo, headers={"Authorization": f"Bearer {gh.githubtoken}"}) as resp:
            if resp.status == 204:
                return True
            if resp.status == 404:
                return False
    ...
    async def __local_check(gh, ctx):
        if gh.githubtoken is None:
            await ctx.send('Github token not provided.', delete_after=10)
            return False
        return True

    @commands.command()
    async def issue(gh, ctx, repo, issueid):
    ...
    @commands.command()
    async def makeissue(gh, ctx, repo, title, *, body):
        '''Create an issue! `{p}makeissue <title> | <body>`'''
        async with ctx.session.post(f'https://api.github.com/repos/{repo}/issues', json={"title": title, "body": body}, headers={'Authorization': f'Bearer {gh.githubtoken}'}) as resp:
            if resp.status == 200 or resp.status == 201:
                issueinfo = await resp.json()
            else:
                return await ctx.send('ConnectionError: Github API Issue.')
    ...
    @commands.command()
    async def comment(gh, ctx, repo, issueid:int, *, content):
        '''Comment on a Github Issue'''
        async with ctx.session.post(f'https://api.github.com/repos/{repo}/issues/{issueid}/comments', json={"body": content}, headers={'Authorization': f'Bearer {gh.githubtoken}'}) as resp:
            if resp.status != 200 and resp.status != 201:
                return await ctx.send('ConnectionError: Github API Issue.')
        await ctx.send('Submitted comment to issue ' + str(issueid))

    @commands.command()
    async def suggest(gh, ctx, summary, *, details):
        '''Create an issue! `{p}makeissue <short summary> | <details>`'''
        async with ctx.session.post('https://api.github.com/repos/kyb3r/ghbot.py/issues', json={"title": summary, "body": details}, headers={'Authorization': f'Bearer {gh.githubtoken}'}) as resp:
            if resp.status == 200 or resp.status == 201:
                issueinfo = await resp.json()
            else:
                return await ctx.send('ConnectionError: Github API Issue.')
